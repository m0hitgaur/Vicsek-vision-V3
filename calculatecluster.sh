...
for angle_folder in "${angle_folders[@]}"; do
  for noise_folder in "${noise_folders[@]}"; do
    ...
    if [ -f "$folder_path/calculate_cluster.cpp" ]; then
      echo "Processing folder: $folder_path"
      # Run commands in background with '&'
      (
        cd "$folder_path"
        g++ calculate_cluster.cpp -o ci.out
        if [ $? -ne 0 ]; then
          ...
          if [ -f ci.out ]; then
            ...
            echo "Error: ci.out not found after compilation in $folder_path"
          fi
        fi
        cd "$current_dir"
      ) &
    else
      echo "calculate_cluster.cpp not found in $angle_folder/$noise_folder"
    fi
  done
done
