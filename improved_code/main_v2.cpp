...
public:
    ...
    void velocity_alignment_long_range(double alpha,double eta) {    
        ...
        for (int i=0;i<particles.size();i++) {
            ...
            {   
                ...
                dx=minimum_image(particles[j].x - particles[i].x,Lx);
                dy=minimum_image(particles[j].y - particles[i].y,Ly);                 
                double rij = hypot(dx,dy);
                vector<int> count_s(N,0);
                ...
                if(count_s[i]<long_neighbours  && rij>=rc && rij<=2*rc && innerproduct_i >= cos(alpha) ) 
                    {avg_vy[i]+= sin(theta_j);
                    avg_vx[i]+=cos(theta_j);
                    count_v[i]++;}
                
                if(count_s[j]<long_neighbours && rij<=rc && rij<=2*rc && innerproduct_j >= cos(alpha) ) 
                    {avg_vy[j]+= sin(theta_i);
                    avg_vx[j]+=cos(theta_i);
                    count_v[j]++;}   
    ...
    void velocity_alignment_long_range_topological(double alpha,double eta) {    
        vector<double> avg_vx(N,0);
        vector<double> avg_vy(N,0);
        double newtheta;
        vector<int> count_v(N,1); // starting from 1 as the particle itself is always counted
    ...
    void start_run() {
        ...
        for (int t = 0; t < tmax; t++) {             
            ...
            velocity_alignment_long_range(half_angle,noise);
            velocity_update();
            position_update(); 
            EndTimeStep();
...
int main() { 
    double noise=5.0e-2 ;     // strength of noise
