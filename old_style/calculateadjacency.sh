...
for angle_folder in "${angle_folders[@]}"; do
  for noise_folder in "${noise_folders[@]}"; do
    ...
    if [ -f "$folder_path/calculateadjacency.cpp" ]; then
      echo "Processing folder: $folder_path"
      # Run commands in background with '&'
      (
        cd "$folder_path"
        g++ calculateadjacency.cpp -o adj.out
        if [ $? -ne 0 ]; then
          ...
          if [ -f adj.out ]; then
            ...
            echo "Error: adj.out not found after compilation in $folder_path"
          fi
        fi
        cd "$current_dir"
      ) &
    else
      echo "calculateadjacency.cpp not found in $angle_folder/$noise_folder"
    fi
  done
done
