...
int main() {
    ...
    {   trial_time=time(NULL);
        ...
        //update theta implemented from paper 
            ...
            vector<int> count_v(Number_of_active_agents,1); // starting from 1 as the particle itself is always counted
                ...
                {   double dx=position[0][j] - position[0][i];
                  ...
                  /*if(dx>Lx/2) dx=dx-Lx;
                    ...
                    */    
                    ...
                    if(count_s[i]<long_int_number && rij[i][j] > radius && rij[i][j] <= 2*radius && innerproduct_i >= cos(half_angle) ) 
                        {avg_sy[i]+= sin(theta[j]);
                        avg_sx[i]+=cos(theta[j]);
                        count_s[i]++;}
                    
                    if(count_s[j]<long_int_number && rij[i][j] > radius &&rij[j][i] <= 2*radius && innerproduct_j >= cos(half_angle) ) 
            ...
            double Sx=w*avg_vx[i] + (1-w)*avg_sx[i];
            double Sy=w*avg_vy[i] + (1-w)*avg_sy[i];

            newtheta[i] = atan2(Sy,Sx) + ((double(rand()%1000)/1000)*(noise))-(noise/2);
