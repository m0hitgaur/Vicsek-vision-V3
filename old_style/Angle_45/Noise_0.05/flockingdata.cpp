        ...
        //update theta implemented from paper 
            ...
            for (int i = 0; i < Number_of_active_agents; i++) {
                ...
                {   double dx=position[0][j] - position[0][i];
                    ...
                    int nn=0;                                            
                    if(count_s[i]<nn && rij[i][j] > radius && rij[i][j] <= 2*radius && innerproduct_i >= cos(half_angle) ) 
                        {avg_vy[i]+= sin(theta[j]);
                        avg_vx[i]+=cos(theta[j]);
                        count_v[i]++;}
                    
                    if(count_s[j]<nn && rij[i][j] > radius &&rij[j][i] <= 2*radius && innerproduct_j >= cos(half_angle) ) 
                        {avg_vy[j]+= sin(theta[i]);
                        avg_vx[j]+=cos(theta[i]);
                        count_v[j]++;}                    
            ...
            newtheta[i] = atan2(avg_vy[i],avg_vx[i]) + ((double(rand()%1000)/1000)*(noise))-(noise/2);
